...
- run `distr maintenance encrypt-database` once, either from a shell in the Hub container or through the
  `database-encryption` CronJob that the Helm chart ships, or
- set `DATABASE_ENCRYPTION_MIGRATE_ON_BOOT=true` and restart the Hub, which does the same during startup.

Either way it is safe to run while the Hub is serving traffic, and it can be interrupted and resumed. Until it has
finished, the Hub logs a warning on every start listing the columns that are still unencrypted.
