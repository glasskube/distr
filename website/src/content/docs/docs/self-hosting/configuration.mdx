...
Instead of the key itself, the variable can also hold a ciphertext that the Hub unwraps with AWS KMS or GCP Cloud KMS
while it starts, so that no key material is kept in your configuration — see
[Key Management Service](/docs/self-hosting/key-management/).
...
Unwraps `JWT_SECRET` and `DATABASE_ENCRYPTION_KEY` with AWS KMS or GCP Cloud KMS during startup, so that no key
material is kept in your configuration.
See [Key Management Service](/docs/self-hosting/key-management/) for details.
